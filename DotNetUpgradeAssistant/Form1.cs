...
namespace DotNetUpgradeAssistant
{
    public sealed partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            cboTarget.Items.AddRange(FrameworkCatalog.FriendlyNames.ToArray<object>());
            if (cboTarget.Items.Count > 0) cboTarget.SelectedIndex = 1; // .NET 8.0 by default
        ...
        private void LoadProjects(string root)
        {
            ...
            foreach (string csproj in files)
            {
                ...
                {
                    Path = csproj,
                    CurrentTfm = normalizedTfm, // keep normalized values
                    Sdk = friendly,
                    Node = node
                });
            }
        ...
        private async void btnUpgrade_Click(object? sender, EventArgs e)
        {
            ...
            string backupDir = Path.Combine(_solutionRoot, "Backups",
            ...
            /* ---------- Stage 2: changing SDK and restore ---------- */
            ...
            MessageBox.Show(_hadNugetIssues ? "Projects upgraded, but some NuGet packages are incompatible.\n" + "Please review them manually."
                    : "Projects upgraded successfully.", "Upgrade result", MessageBoxButtons.OK, _hadNugetIssues
                    ? MessageBoxIcon.Warning : MessageBoxIcon.Information);

            // load again for update treeView
            LoadProjects(_solutionRoot);
        ...
        private static async Task<(string stdout, string stderr, int exitCode)> RunDotnetAsync(string args)
        {
            ...
            {
                if (e.Data is not null) sbOut.AppendLine(e.Data);
            };
            p.ErrorDataReceived += (_, e) =>
            {
                if (e.Data is not null) sbErr.AppendLine(e.Data);
            };
